...
export const WithFriendsActionService = {
    ...
    explodeWallWithFriends(datas, callback){
        console.log("DANS EXPLODE WALL WITH FRIENDS")
        ...
        console.log(reqSerialized);
        socketManager.socket.emit('explodeWallWithFriends',reqSerialized);

        // Écouter la réponse du serveur sur la même socket
        socketManager.socket.once('explodeWallWithFriendsResponse', (res) => {
            console.log("EXPLODE WALL WITH FRIENDS RESPONSE");
            callback(res);
        });
    },
